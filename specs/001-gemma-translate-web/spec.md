...
4. **Given** 翻譯服務後端未啟動，**When** 使用者送出翻譯請求，**Then** 系統應顯示清楚的錯誤訊息而非空白或崩潰
...
1. **Given** 使用者開啟網頁，**When** 使用者點擊語言下拉選單，**Then** 系統應顯示至少 15 種常用語言選項
2. **Given** 使用者選擇英文為來源語言、中文為目標語言，**When** 使用者輸入英文文字並送出，**Then** 系統應返回中文翻譯
3. **Given** 使用者選擇相同的來源和目標語言，**When** 送出翻譯請求，**Then** 系統應顯示友善提示或自動複製原文
4. **Given** 使用者完成一次翻譯，**When** 使用者切換語言對並再次翻譯，**Then** 系統應根據新的語言對產生正確翻譯
...
4. **Given** 對話區域顯示多筆記錄，**When** 頁面佈局調整，**Then** 對話泡泡應左右對齊（原文左，譯文右）並保持可讀性
...
1. **Given** 使用者送出翻譯請求，**When** 後端開始回傳結果，**Then** 翻譯文字應逐步出現而非一次性顯示完整內容
...
- **空白輸入**：使用者未輸入任何文字就點擊送出時，系統應顯示提示訊息而非發送請求
- **特殊字元**：使用者輸入包含 emoji、符號、換行符號的文字時，系統應正確處理並保留格式
- **超長文字**：使用者輸入超過模型處理上限的文字時，系統應顯示友善提示或自動截斷並告知使用者
- **翻譯逾時**：模型推論超過 120 秒仍未完成時，系統應中斷請求並提示使用者重試
- **模型未載入**：後端服務啟動但模型尚未載入完成時，翻譯請求應回傳明確的「模型初始化中」訊息
- **不支援的語言對**：使用者選擇的語言對模型不支援時，系統應在送出前或回傳時告知使用者
...
- **FR-002**: 系統必須提供來源語言和目標語言的選擇介面，至少支援 15 種常用語言（包含英文、中文、日文、韓文、法文、德文、西班牙文、葡萄牙文、俄文、義大利文、阿拉伯文、印地文、泰文、越南文、印尼文）
- **FR-003**: 系統必須在使用者送出翻譯請求後，將原文送至後端 API 進行翻譯
- **FR-004**: 系統必須以對話式介面呈現翻譯結果，原文與譯文分別顯示在左右兩側
- **FR-005**: 系統必須支援串流方式接收並顯示翻譯結果
...
- **語言 (Language)**: 代表系統支援的語言，包含語言碼（如 "en"、"zh-TW"）和語言名稱（如 "English"、"Chinese"）
