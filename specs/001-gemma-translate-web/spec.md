...
- **FR-017**: 系統必須提供雙層錯誤訊息顯示機制：輕量錯誤（如空白輸入、格式驗證失敗）透過 Toast 通知顯示 3-5 秒後自動消失；嚴重錯誤（如翻譯失敗、後端無回應、逾時）在對話區域顯示錯誤訊息泡泡並保留在歷史記錄中- **FR-018**: 系統必須支援自訂術語對照表功能,允許使用者定義特定原文詞彙與其對應譯文,翻譯時優先使用術語對照表中的對應翻譯
### Key Entities
